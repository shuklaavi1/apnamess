...
export function AddPurchaseModal({ isOpen, onClose }: AddPurchaseModalProps) {
  ...
  // Ensure paidByMemberId is kept in sync with loaded members or currentMember
  React.useEffect(() => {
    if (isOpen) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!itemName.trim()) {
      alert('Please specify what you bought');
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/50 backdrop-blur-xs p-0 sm:p-4">
      <div className="bg-white border border-slate-200 text-slate-900 w-full max-w-md rounded-t-2xl sm:rounded-xl shadow-xl p-5 animate-in slide-in-from-bottom duration-150">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">What did you buy? *</label>
            <input
              type="text"
              required
              placeholder="e.g. Vegetables, Milk, Oil"
              value={itemName}
