...
export default function MemberDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!member) {
    return (
      <div className="py-8 text-center space-y-3 text-xs">
        <div className="font-semibold text-slate-800">Member Not Found</div>
  ...
  // Combine member's history (contributions + expenses paid by this member)
  ...
  return (
    <div className="space-y-6">
      <Link href="/members" className="text-xs text-blue-600 font-semibold hover:underline">
        ← Back to Members
      </Link>

      <div className="border-b border-slate-200 pb-3">
        <h1 className="text-xl font-bold text-slate-900">{member.display_name}</h1>
      </div>
      ...
      <div className="space-y-3">
        ...
        {history.length === 0 ? (
          <div className="text-xs text-slate-400 py-4 text-center">No history for this month yet.</div>
        ) : (
            ...
            {history.map((item) => (
              <div key={item.id} className="py-2.5 px-3 flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  <span className="text-slate-500 w-14 font-mono text-[11px]">{item.date}</span>
